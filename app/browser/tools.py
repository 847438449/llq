...
async def open_url(page, url: str) -> dict:
    page.goto(url, wait_until="domcontentloaded")
    return {"title": page.title()}


async def search_text(page, text: str) -> dict:
    content = page.content()
    found = text.lower() in content.lower()
    return {"query": text, "found": found}


async def click_text(page, text: str) -> dict:
    locator = page.get_by_text(text).first
    locator.click(timeout=5000)
    return {"clicked_text": text}


async def extract_page_summary(page, max_chars: int = 400) -> dict:
    title = page.title()
...
async def take_screenshot(page, path: Path, timeout_ms: int = 10000, full_page: bool = False) -> dict:

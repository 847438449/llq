...
class RuleBasedPlanner:
    """State-aware planner with LLM next-action planning and deterministic fallback."""
    ...
    def next_action(
        ...
        try:
            ...
            debug = {
                ...
                "planner_raw_output": f"fallback:{type(exc).__name__}",
                "validated_action": step.model_dump_json(),
                "planner_reasoning_summary": "Used deterministic fallback because LLM output was invalid/unavailable.",
    ...
    def llm_next_action(
        ...
        response = client.responses.create(
            ...
            input=[
                {"role": "system", "content": self.prompt_template},
                {
                    "role": "user",
                    "content": (
                        "User task and context for next action planning:\n"
        ...
        debug = {
            ...
            "validated_action": step.model_dump_json(),
            "planner_reasoning_summary": "Used LLM next-action planning with skill and past experience context.",
            "past_experience_used": past_experience,
        }
        return step, debug

    def naive_next_action(
        self,
        task: str,
        page_state: dict,
        action_history: list[dict],
    ...
    def _input_summary(self, task: str, page_state: dict, skill_context: dict) -> str:
        ...
        skill = skill_context.get("name", "generic_web")
        return f"task={task[:80]} | url={url} | title={title[:60]} | skill={skill}"
